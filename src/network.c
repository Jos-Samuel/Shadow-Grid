    ...
    while (1) {
        ...
        while ((newline = strchr(line_start, '\n')) != NULL) {
            *newline = '\0';
            ...
            case EVENT_ROLE: {
                if (player_id < 0) snprintf(reply, sizeof(reply), "RESP:ERR;MSG:JOIN_FIRST\n");
                    ...
                    if (strcasecmp(e.dir, "PLAYER") == 0) { players[player_id].role = ROLE_PLAYER; snprintf(reply, sizeof(reply), "RESP:OK;MSG:ROLE_UPDATED\n"); }
                        ...
                        if (player_id == 0 || count_active_players() == 1) { players[player_id].role = ROLE_ADMIN; snprintf(reply, sizeof(reply), "RESP:OK;MSG:ROLE_UPDATED\n"); }
            ...
            if (e.type == EVENT_QUIT) { close(client_socket); return NULL; }
            line_start = newline + 1;
        }
